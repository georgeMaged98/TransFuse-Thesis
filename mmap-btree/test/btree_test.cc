...
using BufferFrame = transfuse::BufferFrame;
using BufferManager = transfuse::BufferManager;
using Defer = transfuse::Defer;
using BTree = transfuse::BTree<uint64_t, uint64_t, std::less<uint64_t>, 1024>;

char * test_filename = "/media/george-elfayoumi/Academic/TUM/TransFuse-Thesis/mmap-btree/btree_test.txt";
//char * test_filename = "/tmp/transfuse_mnt/btree_test.txt";
...
namespace {
    ...
    //     using BTree = transfuse::BTree<uint64_t, uint64_t, std::less<uint64_t>, 1u << 16>;
    ...
    //     transfuse::FileMapper file_mapper(test_filename, 4096);
    //     BTree tree(0, file_mapper);
    //     tree.insert(42, 21);
    //
    //     auto test = "inserting an element into an empty B-Tree";
    //     auto *root_page = file_mapper.get_page(tree.root, false);
    ...
    //     transfuse::FileMapper file_mapper(test_filename, page_size);
    //     BTree tree(0, file_mapper);
    //
    ...
    //     auto root_leaf_node = static_cast<BTree::LeafNode *>(root_node);
    //     ASSERT_EQ(root_leaf_node->count, BTree::LeafNode::kCapacity)
    ...
    //     transfuse::FileMapper file_mapper(test_filename, 1024);
    //     BTree tree(0, file_mapper);
    //
    //     for (auto i = 0ul; i < BTree::LeafNode::kCapacity; ++i) {
    //         tree.insert(i, 2 * i);
 ...
 //            ASSERT_TRUE(v)
 //             << "searching for the just inserted key k=" << rand_key << " after i=" << (i - 1) <<
 // " inserts yields nothing";
 //            ASSERT_EQ(*v, i)
