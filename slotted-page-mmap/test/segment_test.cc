 ...
 using FileMapper = moderndbs::FileMapper;
 ...
 TEST_F(SegmentTest, SchemaSerialiseEmptySchema) {
    ...
    SchemaSegment schema_segment_2(0, schema_file_mapper);
 ...
 TEST_F(SegmentTest, SPRecordWriteRead) {
    ...
    for (uint64_t size = 1; size < max; size *= 2) {
       ...
       sp_segment.write(tid, writeBuffer.data(), size, 1);
 ...
 TEST_F(SegmentTest, SPRecordWriteReadRedirect) {
    ...
    for (uint64_t i = 0; i < max_records; ++i) {
       ...
       tids.push_back(tid);
       sp_segment.write(tid, reinterpret_cast<std::byte*>(&i), record_size, 1);
       uint64_t x = 0;
 ...
 TEST_F(SegmentTest, SPRecordErase) {
    ...
    // Erase the slot
    sp_segment.erase(tid, 1);

    // Reload the page
    page_id = tid.get_page_id(table.sp_segment);
    frame = sp_file_mapper.get_page(page_id, true);
