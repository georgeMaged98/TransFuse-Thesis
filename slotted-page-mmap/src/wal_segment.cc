...
WALSegment::WALSegment(FileMapper& file_mapper)
   : Segment(segment_id, file_mapper), stop_background_flush(false) {
   std::unique_lock lock(wal_mutex);

   std::string filename = file_mapper.filename_;
   bool is_new = false;
...
void WALSegment::append_to_WAL_Segment(const char* data, size_t data_size) {
   std::unique_ptr<File> file = File::open_file(file_mapper.filename_.c_str(), File::Mode::WRITE);
