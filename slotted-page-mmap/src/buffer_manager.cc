...
void BufferManager::write_buffer_frame_to_file(BufferFrame* page) const {
   auto segment_id = BufferManager::get_segment_id(page->pageNo);
   try {
      std::unique_ptr<File> file = File::open_file((std::to_string(segment_id) + "").c_str(), File::Mode::WRITE);
...
void BufferManager::read_buffer_frame_from_file(uint64_t page_id, BufferFrame& bf) const {
   ...
   try {
      std::unique_ptr<File> file = File::open_file((std::to_string(segment_id) + "").c_str(),
                                                   File::Mode::READ);
...
void BufferManager::evict_page() {
   std::unique_lock<std::mutex> bf_lock(buffer_manager_mutex);
