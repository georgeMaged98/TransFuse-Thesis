...
moderndbs::TID moderndbs::Database::insert(const schema::Table &table,
   ...
   latch.lock();
    ...
    const auto tid = sp.allocate(insert_buffer.size());
    sp.write(tid, reinterpret_cast<std::byte *>(insert_buffer.data()), insert_buffer.size());
    std::cout << "Tuple with TID " << tid.get_value() <<  " Page: " << tid.get_page_id(table.sp_segment) << " Slot: "  << tid.get_slot() <<" inserted!\n";
    latch.unlock();
    return tid;
}
...
std::optional<std::vector<std::string>> moderndbs::Database::read_tuple(const schema::Table &table, const TID tid) {
   latch.lock();
