...
class BufferManager {
   ...
   std::shared_ptr<BufferFrame> fix_page(uint64_t page_id, bool exclusive);
   ...
   void unfix_page(std::shared_ptr<BufferFrame> page, bool is_dirty);

   /// Takes a `BufferFrame` reference and writes it to disk
   void write_buffer_frame_to_file(uint64_t pageNo, char* data) const;
   ...
   [[nodiscard]] std::shared_ptr<BufferFrame> find_buffer_frame_by_page_id(uint64_t page_id);
