...
/// Let's consider 1.1 GB for 1 GB data. (1 GB = 1 * 1024 * 1024 KB) divided by 4 (KB)
/// to get number of buffer frame pages that cover whole dataset.
constexpr static int max_BM_size = (1 * 1024 * 1024) / 4;
// constexpr static uint64_t max_num_records = 20971520; /// For Reference
static std::vector<uint64_t> max_num_records = {50000, 100000, 500000}; // 100,000  - 1,000,000
...
constexpr static double cold_data_percent = 0.05;
...
void hot_cold_operations_DB(moderndbs::Database& db, const uint64_t operations_num, const double cold_data_percent, const double read_percent) {
   ...
   auto end = std::chrono::high_resolution_clock::now();
   auto duration = std::chrono::duration_cast<std::chrono::seconds>(end - start);
   std::cout << "FINITO HOT COld - Time taken: " << duration.count() << " seconds - Read % " << read_data_percent_hot_cold_test << " Num Operations: " << operations_num
